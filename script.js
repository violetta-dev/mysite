...
// v2.7.0 — enhancements
// Hamburger/console collapse (mobile)
(function(){
  const consoleBox = document.querySelector('.console');
  if(!consoleBox) return;
  const btn = document.createElement('button');
  btn.className = 'console-toggle-btn btn neon';
  btn.type='button';
  btn.textContent = '☰ Menu';
  document.body.appendChild(btn);
  const apply = () => {
    if(window.innerWidth <= 900) consoleBox.classList.add('collapsed');
    else consoleBox.classList.remove('collapsed');
  };
  apply();
...
(function(){
  ...
  wrap.innerHTML = '<a href="#top" class="btn neon">↑ Torna su</a>';
  document.body.appendChild(wrap);
  const onScroll = ()=>{
    if(window.scrollY > 280) wrap.classList.add('show'); else wrap.classList.remove('show');
  };
  window.addEventListener('scroll', onScroll); onScroll();
  wrap.addEventListener('click', (e)=>{
    if(e.target.closest('a')){ e.preventDefault(); window.scrollTo({top:0, behavior:'smooth'}); }
  });
})();

// Timeline GSAP highlight
window.addEventListener('load', ()=>{
  if (typeof gsap === 'undefined' || typeof ScrollTrigger === 'undefined') return;
  gsap.utils.toArray('.tl-item').forEach((el)=>{
    gsap.fromTo(el, {opacity:0.25, x:-12}, {opacity:1, x:0, duration:.5, ease:'power2.out',
      scrollTrigger:{trigger:el, start:'top 80%', toggleActions:'play none none reverse'}});
  });
});

// Projects loader: add ALL tab, fix path to projects.json
async function loadProjects_v27(){
  try{
    ...
    const wrap=document.getElementById('project-cards'); const tabs=document.getElementById('tabs'); if(!wrap||!tabs) return;
    const states = new Set(list.map(p=>p.status));
    const wantTabs=[['all','ALL'],['active','ACTIVE'],['completed','COMPLETED'],['deprecated','DEPRECATED']];
    if(states.has('in_development')||states.has('posticipated')) wantTabs.push(['in_dev','IN DEVELOPMENT']);
    if(states.has('annullato')||states.has('updated')) wantTabs.push(['ann_upd','ANNULLATI / UPDATED']);
    // Render tabs
    tabs.innerHTML='';
    ...
    function badgeLabel(s){ return ({'active':'ACTIVE','completed':'COMPLETED','deprecated':'DEPRECATED','annullato':'ANNULLATO','updated':'UPDATED','in_development':'IN DEVELOPMENT','posticipated':'POSTICIPATED'})[s]||s.toUpperCase(); }
    function groupOf(s){ if(['active','completed','deprecated'].includes(s)) return s; if(s==='in_development'||s==='posticipated') return 'in_dev'; if(s==='annullato'||s==='updated') return 'ann_upd'; return 'other'; }
    wrap.innerHTML='';
    for(const p of list){
      const card=document.createElement('article'); card.className='card'; card.dataset.status=p.status||'completed'; card.dataset.group=groupOf(p.status||'completed');
      const tags = (p.tags||[]).map(t=>`<span>#${t}</span>`).join(' ');
      const link = p.link ? `<p><a class="btn ghost" href="${p.link}" target="_blank" rel="noopener">Apri</a></p>` : '';
      card.innerHTML=`<span class="badge">${badgeLabel(p.status||'completed')}</span>
        <h3>${p.title}</h3>
        <div class="meta">${p.year||''}</div>
        <p>${p.description||''}</p>
        <div class="tags">${tags}</div>${link}`;
      wrap.appendChild(card);
    }
    function applyFilter(f){
      document.querySelectorAll('.tab').forEach(t=>t.classList.toggle('active', t.dataset.filter===f));
      document.querySelectorAll('.card').forEach(c=>{
        c.style.display = (f==='all') ? 'block' : ((c.dataset.group===f || c.dataset.status===f) ? 'block':'none');
      });
    }
    tabs.querySelectorAll('.tab').forEach(btn=>btn.addEventListener('click',()=>applyFilter(btn.dataset.filter)));
    applyFilter(wantTabs[0][0]);
  }catch(e){ console.error('projects load error v2.7.0',e); }
}
if(document.body.dataset.page==='projects') loadProjects_v27();
