...
// VIX — Neural Core v2.1 (Orbitron Fixed)
(function(){
  // Intro persisted using localStorage
  const intro = document.getElementById('intro');
  if (intro) {
    const seen = localStorage.getItem('vixIntroSeen');
    if (seen) {
      intro.remove();
    } else {
      const hideIntro = () => { intro.classList.add('hide'); localStorage.setItem('vixIntroSeen','true'); };
      const enter = document.getElementById('enterBtn');
      if (enter) enter.addEventListener('click', hideIntro);
      setTimeout(hideIntro, 2200);
    }
  }

  // Dynamic projects with optional thumbnails
  async function loadProjects(){
    ...
    try{
      ...
      grid.innerHTML = list.map(p => {
        ...
        const img = (p.thumb || '').trim();
        const imgHTML = img ? `<img src="${img}" alt="${p.title||'Progetto'}" class="thumb" loading="lazy">` : '';
        return `<article class="card project" data-tags="${slug}">
          ${imgHTML}
          <h3>${p.title||'Progetto'}</h3>
          <p>${p.description||''}</p>
          <div class="tags">${tgs}</div>
          <button class="btn" data-role="open" data-title="${(p.title||'')}" data-desc="${(p.description||'')}" data-tags="${(p.tags||[]).join(', ')}" data-thumb="${img}">Dettagli</button>
        </article>`;
      }).join('');
      ...
      document.addEventListener('click', e => {
        ...
        if(btn){
          ...
          const th = (btn.dataset.thumb||'').trim();
          mMedia.innerHTML = th ? `<img src="${th}" alt="${btn.dataset.title||'Progetto'}">` : '';
